...
const AllowMentioned = ({info, asyncInfo}) => {
  ...
  const {underLine, content, descriptionText} = styles();
  const renderItem = ({item}) => {
    const value = item.title === asyncInfo[info];
    return (
      <MenuRadio
        title={`privacy.mentioned.${item.title}`}
        value={value}
        onPress={() => {
    ...
    <View>
      ...
      <View style={content}>
        <Text style={descriptionText}>
          {i18n.t('privacy.mentioned.content')}
        </Text>
      </View>
    </View>
  );
